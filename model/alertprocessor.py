...
class AlertProcessor:
            ...
            for (roi_name, value) in self.rois.items():
                ...
                if roi_max_alert > 0:
                    ...
                    for town_name, town_geom in value['towns_10k'].items():
                        ...
                        if town_max_alert > 0:
                            alerts.append(
                                (roi_name, town_name, town_max_alert))

                # yield the alerts
                for emails in (r.emails for r in self.recipients
                               if roi_name.lower() in r.rois):
                    for email in emails:
                        yield {"email": email,
                               "roi": roi_name,
                               "towns": alerts}
    ...
    def load_recipients(self, db_path: Union[str, Path]):
        ...
        def _read_from_csv(csv_file: Union[str, Path]) -> Iterable[Recipient]:
            with open(csv_file) as input_file:
                ...
                missing_keys = set(Recipient.csv_fields).difference(
                    reader.fieldnames)
                if any(missing_keys):
                    raise Exception("One or more fields could not be found in the CSV file.\n"
        ...
        if user_db.suffix == ".csv":
            self.recipients.extend([r for r in _read_from_csv(db_path)])
    ...
    def load_rois(self, rois: Iterable[Union[str, Path]]):
        ...
        for path in (Path(p) for p in rois):
            if path.exists() and path.is_file():
                self.rois.update((name, {"geometry": geom, 'towns': {}, 'towns_10k': {}})
                                 for (name, geom) in AlertProcessor._load_geometries(path))

    def load_towns(self, path: Union[str, Path]):
        """Loads the towns and communities.

        Args:
            path (Union[str, Path]): Path to the towns and communities geometric data.
        """
    ...
    def _load_geometries(path: Union[str, Path]):
        ...
        assert "name" in gdf.columns, \
            "There is no <Name> field in the attribute table.\n" + \
            "File name: " + path.resolve()
        assert "geometry" in gdf.columns, \
            "The file does not contain any geometric data.\n" + \
            "File name: " + path.resolve()

        # yield the rows
        for (name, geom) in ((str(row[1]['name']), row[1]['geometry'])
