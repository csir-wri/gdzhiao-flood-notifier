...
class Recipient:
    ...
    def __init__(self, **kwargs):
        ...
        self._rois = tuple(roi.strip().lower()
                           for roi in str(kwargs['rois']).split(";"))
        self._expertise = Expertise(int(kwargs["expertise"]))
        self._emails = tuple(email.strip()
                             for email in str(kwargs["email"]).split(";"))
        self._phone_nums = tuple(num.strip()
                                 for num in str(kwargs["phone"]).split(";"))
    ...
    @property
    def emails(self) -> typing.Tuple[str]:
        """Gets this recipient's email addresses.

        Returns:
            typing.Tuple[str]: Email addresses of the recipient.
        """
        return self._emails

    @property
    def phone_numbers(self) -> typing.Tuple[str]:
        """Gets this recipient's phone numbers.

        Returns:
            typing.Tuple[str]: Phone numbers of the recipient.
        """
        return self._phone_nums

    @property
    def rois(self) -> typing.Tuple[str]:
        """Gets the IDs of this recipient's regions of interest.

        Returns:
            typing.Tuple[str]: Tuple containing the IDs of this recipient's
            regions of interest.
        """
        return self._rois
